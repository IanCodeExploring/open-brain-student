...
//
// Paste a YouTube link, get the transcript captured and saved. No AI summary
// yet (that arrives in Level 5) — this saves the raw transcript/description.
  ...
  try {
    ...
    if (!result) {
      return jsonResponse({
        ok: false,
        error:
          "Could not read anything from that video. It may be private, age-restricted, or region-locked. Try a different one.",
      }, 422);
    }
    ...
    if (insertError || !thought) {
      return jsonResponse({ ok: false, error: insertError?.message ?? "Failed to save thought" }, 500);
    }
    ...
    if (sourceError) {
      console.error("[capture-youtube] thought_sources insert failed:", sourceError.message);
    }
