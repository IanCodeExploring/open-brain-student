...
/**
 * Adapted for this project: embeddings are generated through our own generate-embedding
 * edge function (Level 6) rather than the Express kit's _shared/ai.ts helper. Same idea as
 * generateEmbedding, just called over HTTP with the service role key instead of imported
 * directly, since this file lives outside that function.
 */
async function generateEmbedding(text: string, _opts?: { userId?: string; source?: string }): Promise<number[] | null> {
  try {
    ...
    if (!embResponse.ok) {
      console.error(`[thought-chunks] generate-embedding returned ${embResponse.status}`)
      return null
    }
    const { embedding } = await embResponse.json()
    return embedding ?? null
  } catch (err) {
    console.error('[thought-chunks] generate-embedding call failed:', String(err))
    return null
  }
}
